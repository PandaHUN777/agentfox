...
def _src_text() -> str:
    return "\n".join(
        p.read_text(errors="ignore") for p in SRC.rglob("*.py")
    )
...
PROBES: list[Probe] = [
    # --- Layer A: Know
    Probe("P1", "Registry, shadow discovery, observed lineage", "1 Registry",
    ...
    Probe("P2", "NHI, least privilege, delegation narrowing, approvals", "2 Identity",
          ...
          ["def link_external_identity"], "capability|delegation|approval",
          "no live IdP; Entra/Okta integration (P2-8) absent"),
    Probe("P3", "Runtime detectors across five surfaces + taint", "3 Guardrails",
          ["class InjectionHeuristicDetector", "class NativePiiDetector",
    ...
    # --- Layer C: Ground
    Probe("P7", "Knowledge boundary, forced abstention", "7 Answerability",
          ["class KnowledgeBoundary"], [], "answerab|abstain"),
    Probe("P8", "Source tiers, freshness, citation binding", "8 Provenance",
          ["def groundedness"], ["class SourceRegistry"], "groundedness|provenance|citation",
          "lexical groundedness only; source authority absent"),
    Probe("P14", "Ingestion and retrieval quality gates", "14 Context Integrity",
          ["def chunk_quality"], [], "chunk_quality|retrieval_metric"),
    # --- Layer D: Judge
    Probe("P4", "Eval runner, CI gating, drift, silent failure, red team", "4 Evaluation",
          ["def gate", "class SilentFailureScorer", "def compute", "def run_campaign"],
          ["class RagasAdapter", "def annotation_queue"], "gate|silent_failure|drift|campaign",
          "no Ragas adapter, model-based groundedness or annotation queue"),
    Probe("P13", "Failure attribution across handoffs", "13 Failure Attribution",
          ["def attribute_failure"], [], "attribution|handoff_fidelity"),
    Probe("P11", "Escalation policy and missed-escalation detection", "11 Escalation",
          ["def request_approval"], ["def detect_missed_escalation"],
          "escalation", "approvals exist; missed-escalation detection absent"),
    # --- Layer E: Prove
    Probe("P5", "Traces, hash chain, evidence packages, SIEM", "5 Audit",
          ["def verify", "def build", "def to_cef", "def full_trace"], [],
          "chain|evidence|trace|siem"),
    Probe("P6", "Control catalog, computed status, risk, obligations", "6 Compliance",
          ["def compute_all", "def sync_catalog", "def obligation_calendar"],
          ["def dynamic_risk_score", "class Assessment"], "control|framework|compliance|risk",
    ...
    Probe("PL-1", "Streaming with inline enforcement", "Platform",
          ...
          "stream"),
    Probe("PL-2", "Database migrations", "Platform",
          ["def upgrade_db", "def current_revision"], [], "migration"),
    Probe("PL-3", "Kill switch and quarantine", "Platform",
          ["class AgentControl", "def _control_verdict"], [], "kill|quarantine|control_"),
    Probe("PL-4", "Agent loop governance", "Platform", ["def govern_loop"], [], ""),
    Probe("PL-5", "Async workers", "Platform", ["class JobQueue"], [], ""),
    Probe("PL-6", "HA-ready persistence", "Platform", ["def configure_pool"], [], "",
    ...
    Probe("I-4", "LangSmith correlation", "Integration", ["class LangSmithBridge"], [], ""),
    Probe("I-5", "OpenTelemetry", "Integration", ["def ingest_otlp"], [], "otlp"),
    Probe("I-6", "Langfuse correlation", "Integration", ["class LangfuseBridge"], [], ""),
    ...
    Probe("I-11", "Azure / Bedrock / Vertex providers", "Integration",
          ["class BedrockProvider"], [], "", "OpenAI + Anthropic only"),
]
