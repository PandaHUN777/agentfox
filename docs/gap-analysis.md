...
| 0.4 | **Agent tool-calling loop not governed** — proxy forwards `tools`/`tool_calls` but does not enforce across the multi-turn loop | 🔴 | M | 2 refs only | ◐ **stub-only** — `agent_loop.py` built and tested; zero callers on the live request path |
| 0.5 | **Everything synchronous** — evidence build, red-team, compliance compute run in-request | 🔴 | M | no queue/scheduler | ◐ **stub-only** — `jobs.py` built and tested; nothing enqueues to it live |
...
| 0.7 | No graceful degradation path if the control plane is down (fail-open exists per-detector, not per-service) | 🟠 | S | — | ◐ **stub-only** — `availability.py`'s `FailPolicy`/`DegradationLedger` exist and are tested; not wired to a per-service kill path |

**Net:** the two "production cannot go inline" blockers that mattered most for actually shipping — streaming and migrations — are genuinely closed. The three stub-only items (0.4, 0.5, 0.7) are lower-severity in practice (the system runs fine without them today) but should not be reported as done; they're the same "built the library, didn't plug it in" pattern that recurs across this document and in [failure-modes.md](failure-modes.md)'s F6.
...
| 3.2 | Adaptive / generative red teaming (ours is 11 static probes) | absent | ◐ still 11 native static probes; now wraps NVIDIA Garak / Microsoft PyRIT (`GarakRunner`/`PyritRunner`) when installed — adaptive capability available via optional external tools, not built natively | 🟠 | L |
