...
**Generated by `python scripts/probe/run.py --md`. Do not edit by hand.**

This taxonomy is built from the *architecture* of an agentic request rather than from
[failure-modes.md](failure-modes.md). That catalogue and this codebase co-evolved, so
scoring ourselves against it is circular: it would confirm we cover what we set out to
cover and say nothing about what we never thought of. This one walks the path a request
actually travels and asks, at each layer, what can go wrong there.

**103 scenarios · 67 verified by execution · 69% weighted coverage**
(partial counts half). The harness runs every executable claim against the real product
and fails if any disagrees — so a row marked ✅ here has fired at least once in anger.

| Layer | Score | |
|---|---|---|
| L0 model-intrinsic | 6/11 | `████████` |
| L1 input and prompt | 7/9 | `████████████` |
| L2 retrieval and context | 7/14 | `████████` |
...
| L2.8 | Incoherent chunking | ✗ absent | — | P14-2 specified, not built. |
| L2.9 | Tokeniser failure on non-Latin script | ✗ absent | — | P14-3 specified, not built. |
| L2.10 | Context-window truncation drops the evidence | ✗ absent | — | P14-6 specified, not built. |
| L2.11 | Lost in the middle | ✗ absent | — | P14-6. |
| L2.12 | Memory contamination across sessions | ◐ partial | tenancy isolates storage | Cross-tenant memory is prevented. Within a tenant, cross-session contamination is not detected. P14-7 not buil |
| L2.13 | Retrieval quality drifts over time | ✗ absent | — | P14-4 specified, not built. |
| L2.14 | Corrupt document ingested | ✗ absent | — | P14-1 specified, not built. |
