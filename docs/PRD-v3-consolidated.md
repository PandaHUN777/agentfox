# Nometria — Consolidated PRD v3
## Agent Assurance & Governance for Enterprise AI

| | |
|---|---|
| **Version** | 3.0 — consolidated, self-contained |
| **Date** | 2026-08-18 |
| **Status** | Approved for build |
| **Supersedes** | [PRD v1](PRD.md) (6 pillars) and [PRD v2](PRD-v2.md) (pillars 7–15). Both retained for history; this document is now canonical. |
...
> argument; section 5 is the product; sections 6–11 are the plan.
...
18,700 lines, 178 passing tests, offline-capable. Working: runtime detectors (PII/injection/secrets),
taint tracking, tool-scoped least privilege, delegation narrowing, hash-chained audit with an
independent verifier, evaluation with CI gating, policy engine with simulation, 36 controls mapped to
7 frameworks.

**Not working:** the gateway **silently ignores `stream: true`** (verified) — which alone blocks
inline deployment for most production agents. No migrations, no kill switch, no multi-tenancy
enforcement, no SSO, no LangGraph integration. See §8.
...
regulated enterprises, 2023–2026. Full analysis: [practitioner-signal.md](practitioner-signal.md).
...
| Compliance mapping | never | 36 controls × 7 frameworks |
...
**Our status.** ✅ injection (lexical + structural + provenance-weighted), PII (native + Presidio), secrets, safety lexicon, schema, budgeted concurrent pipeline with degrade-to-observe, per-detector latency telemetry, taint tracking. ◐ classifier weights not bundled.
...
**Our status.** ✗ zero — no end-user identity propagation at all.
...
**Our status.** ◐ approvals and loop-breaking exist; escalation policy and missed-escalation detection do not.
...
**Our status.** ✅ 36 controls × 7 frameworks, computed status with 9 rule kinds, risk classification, obligations, board view, declared gaps per framework. **⚠ all 257 mappings are DRAFT** — excluded from evidence packages by our own gate until a qualified reviewer signs them.
...
|---|---|---|---|---|
| **I-1** | **LangGraph** | **11/11** | Node/edge hooks, checkpointer-aware state, `interrupt()` for HITL, per-node policy binding. **Enforcement must be a LangGraph primitive, not a proxy they route around.** | ✗ |
| **I-2** | **MCP / FastMCP** | **9/11** | Inline governance of the `tools/call` path, not only hygiene scanning | ◐ scan only |
| **I-3** | **FastAPI** | 10/11 | Middleware + dependency for in-process enforcement | ◐ |
| **I-4** | **LangSmith** | **7/11** | Bidirectional: ingest runs, correlate decisions to their trace ids, push scores/verdicts back as feedback | ✗ |
| **I-5** | **OpenTelemetry** | 5/11 | OTLP ingest + export | ✅ |
| **I-6** | **Langfuse** | 4/11 | Ingest traces; push scores and decisions back as observations | ✗ |
| **I-7** | **Prometheus / Grafana** | 4/11 | Metric export into dashboards they already run | ✗ |
| **I-8** | **Ragas** | 3/11 | Scorer adapter — their vocabulary | ✗ |
| **I-9** | **Ray** | 3/11 | Enforcement across Ray actors | ✗ |
| **I-10** | **LiteLLM** | 2/11 | Routing/fallback integration (P15-6) | ✗ |
...
| **I-14** | **DataHub / OpenMetadata / Unity Catalog** | — | Catalog ingestion for source tier, freshness, ownership (P8-9) | ✗ |
...
18,700 lines · 178 passing tests · ruff clean · offline-capable (no API key, no weights, no egress).

✅ Runtime detectors across 5 surfaces · taint tracking with argument provenance · tool-scoped least
privilege with argument constraints · delegation narrowing at write time · hash-chained audit
(4 tamper modes tested, standalone verifier) · evidence packages · evaluation with direction-aware CI
gating · policy engine with immutable versions and traffic simulation · 36 controls × 7 frameworks
with computed status · shadow-agent detection · MCP hygiene · Next.js control plane.

Measured added latency: **2–6 ms** on the heuristic path (budget 100 ms).
...
| 0.4 | **No LangGraph integration** | 11/11 use it — this is the adoption blocker |
| 0.5 | Agent tool-calling loop not governed | 2 refs only |
| 0.6 | Everything synchronous — evidence/red-team/compliance run in-request | no queue |
| 0.7 | No HA validation; SQLite default is single-writer | NFR-3 untested |

**Tier 1 — procurement blockers** (cannot pass security review)

SSO/SCIM ✗ · multi-tenancy not enforced (0 `org_id` filters) ✗ · rate limiting ✗ · KMS/Vault ✗ ·
SOC 2 Type II ✗ · pen test ✗ · DPA/DR/RTO ✗ · dashboard read-only ✗ · no operator audit log ✗.
**11 of 14 standard procurement requirements unmet.**
...
**Tier 3 — capability gaps** — Pillars 7, 9, 10, 12, 13, 14, 15 as specified above.
...
### Tranche 1 — replace the hand-rolled wrapper *(the 6-of-11 opportunity)*
...
`P3-12…P3-14` violation specificity, cumulative latency, false-positive loop
...
### Tranche 3 — depth
`P10` entitlement (highest commercial value, but needs Tranche 0–1 to land first) · `P14` context
integrity · `P8` provenance + catalog ingestion · `P11` escalation · `P4-9…P4-12` Ragas adapter,
...
**Internal:** [PRD v1](PRD.md) · [PRD v2](PRD-v2.md) · [Gap analysis](gap-analysis.md) ·
[Failure modes](failure-modes.md) · [Practitioner signal](practitioner-signal.md) ·
