...
28.2k lines, 391 passing tests, offline-capable, **47% weighted coverage of 32 tracked
capabilities** and **32% of the 50 catalogued failure modes** — computed by probe, not asserted (see [status.md](status.md), regenerated by
...
| Compliance mapping | never | 37 controls × 7 frameworks |
...
**Our status.** ✅ 37 controls × 7 frameworks, computed status with 9 rule kinds, risk classification, obligations, board view, declared gaps per framework. **⚠ all 266 mappings are DRAFT** — excluded from evidence packages by our own gate until a qualified reviewer signs them.
...
**Snapshot — 47% weighted coverage of 32 tracked capabilities** (11 built · 8 partial ·
13 absent) · **32% of the 50 catalogued failure modes** (17 outright, 2 partial) · 28.2k lines ·
391 passing tests · ruff clean · offline-capable (no API key,
...
- **Judge** — evaluation with direction-aware CI gating · silent-failure ensemble · red-team
  harness.
- **Know/Prove** — registry with shadow-agent detection and observed lineage · 37 controls ×
...
The honest reading of these numbers is that **the enforcement spine is real and the coverage breadth
is not**. Eight of fifteen pillars are still partial or absent, and two of the three genuinely
unclaimed capabilities (`P13` failure attribution, `P7` answerability) are not started.
...
integrity · `P8` provenance + catalog ingestion · `P11` escalation ✅ (pulled forward — largest family) · `P4-9…P4-12` Ragas adapter,
