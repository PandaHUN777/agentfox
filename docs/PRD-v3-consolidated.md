...
22.4k lines, 249 passing tests, offline-capable, **34% weighted coverage of 32 tracked
...
36 controls mapped to 7 frameworks. Tranche 0 is complete: streaming, migrations, kill switch and the
...
**Snapshot — 34% weighted coverage of 32 tracked capabilities** (6 built · 10 partial ·
16 absent) · 22.4k lines · 249 passing tests · ruff clean · offline-capable (no API key,
...
- **Tranche 1 (in progress)** — `P12` hierarchical policy composition (`org → team → agent →
  ...
  **pre-flight** as governed events (audit entry + deduplicated finding), not as an HTTP 429
  that disappears into a load-balancer log.

The honest reading of 34% is that **the enforcement spine is real and the coverage breadth
