...
def create_app() -> FastAPI:
    ...
    @app.post("/api/_migrate_more_org_scoped_uniqueness", tags=["platform"])
    def _migrate_more_org_scoped_uniqueness(
        session: Session = Depends(db), user=Depends(current_user)
    ) -> dict[str, Any]:
        """TEMPORARY, run-once — applies migration 217f32001df6 by hand.

        Same reason as the earlier /api/_migrate_control_key_uniqueness: this
        deployment's alembic.ini/migrations/ aren't bundled into the wheel, so
        `nometria db upgrade` can't run against it without the raw Neon connection
        string, which isn't available outside Vercel's env var store. Idempotent —
        safe to call again. Owner/admin only. Remove once applied and confirmed.
        """
        ...
        for table, column, new_name in indexed:
            already = session.execute(
                text("SELECT 1 FROM pg_constraint WHERE conname = :n"), {"n": new_name}
            ).first()
            if already:
                skipped.append(new_name)
                continue
            old_index = f"ix_{table}_{column}"
            session.execute(text(f"DROP INDEX IF EXISTS {old_index}"))
            session.execute(
                text(f"CREATE INDEX IF NOT EXISTS {old_index} ON {table} ({column})")
            )
            session.execute(
                text(f"ALTER TABLE {table} ADD CONSTRAINT {new_name} UNIQUE (org_id, {column})")
            )
            applied.append(new_name)

        for table, column, new_name in bare_unique:
            already = session.execute(
                text("SELECT 1 FROM pg_constraint WHERE conname = :n"), {"n": new_name}
            ).first()
            if already:
